// RSVP.jsx — Vietnamese, prefilled with visitor name
const { useState: useStateRsvp, useEffect: useEffectRsvp } = React;
...
function RSVP({ visitorName }) {
  const [list, setList] = useStateRsvp(() => {
    try { return JSON.parse(localStorage.getItem('bi_rsvp') || 'null') || []; }
    catch { return []; }
  });
  const [name, setName] = useStateRsvp(visitorName || '');
  const [note, setNote] = useStateRsvp('');
  const [status, setStatus] = useStateRsvp('yes');
          ...
          <div className="reveal" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{
              ...
              <div style={{ fontFamily: 'var(--font-sans)', fontSize: 13, color: 'var(--ink-2)' }}>
                <b style={{ color: 'var(--ink)', fontSize: 18, fontFamily: 'var(--font-display)' }}>{list.length}</b> {list.length === 1 ? 'lượt' : 'lượt'} phản hồi
              </div>
              <div style={{ flex: 1 }}/>
