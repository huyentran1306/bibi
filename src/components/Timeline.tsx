...
const TL_COLOR: Record<string, { ring: string; fill: string }> = {
...
interface Milestone { year: string; title: string; desc: string; emoji: string; color: string; highlight?: boolean; }

function TimelineItem({ m, i, isLast }: { m: Milestone; i: number; isLast: boolean }) {
    ...
    <section className="section" id="timeline" style={{ paddingTop: 96 }}>
      ...
      <div style={{ position: 'relative', maxWidth: 920, margin: '40px auto 0' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          {MILESTONES.map((m, i) => (
            <TimelineItem m={m} i={i} isLast={i === MILESTONES.length - 1} key={i}/>
          ))}
        </div>
      </div>
    </section>
